  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen flex flex-col">
      <main className="flex-1">
        ...
        {/* 3. Dynamic Category Showcase Rows */}
        {categoryBuckets.length === 0 ? (
           <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
             <div className="w-full py-12 text-center bg-white rounded-2xl border border-slate-100 p-6">
               <p className="text-slate-500 text-sm">No products found. Please add products from the Admin Dashboard.</p>
             </div>
           </section>
        ) : (
          categoryBuckets.map((category) => (
            <section key={category.id} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold text-slate-900">Trending in {category.name}</h2>
                </div>
                <Link href={`/products?category=${category.id}`} className="hidden sm:flex items-center gap-2 text-emerald-600 font-semibold hover:text-emerald-800 transition-colors">
                  Explore More
                    ...
                    <div className="relative w-full aspect-square bg-slate-50 overflow-hidden rounded-t-2xl">
                      ...
                      {product.images && product.images.length > 1 && (
                        <img 
                          ...
                          className="absolute inset-0 w-full h-full object-cover object-center opacity-0 transition-all duration-500 ease-in-out group-hover:opacity-100 group-hover:scale-105"
                        />
                      )}
                      {/* Overlay Badges */}
              ...
              <div className="mt-8 text-center sm:hidden">
                <Link href={`/products?category=${category.id}`} className="block w-full bg-white border border-slate-200 text-slate-700 font-semibold py-3 px-4 rounded-lg hover:bg-slate-50 transition-colors">
